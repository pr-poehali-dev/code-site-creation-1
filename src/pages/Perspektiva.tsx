...
const included = [
  ...
  { emoji: "🌲", label: "Прогулки по лесу" },
  { emoji: "🔥", label: "Баня с авторскими программами" },
  { emoji: "🤲", label: "Массажи и лимфодренаж" },
  { emoji: "💦", label: "SPA-процедуры" },
  { emoji: "💧", label: "Капельницы и нутрицевтики" },
  { emoji: "🧬", label: "Сдача анализов" },
  { emoji: "🥗", label: "Индивидуальный план питания" },
  { emoji: "😴", label: "Режим сна и циркадные ритмы" },
  { emoji: "🧘", label: "Медитативные практики" },
  { emoji: "🌬️", label: "Дыхательные техники" },
  ...
  { emoji: "🎨", label: "Творческие курсы (глина и др.)" },
];
...
const bioSections = [
  {
    ...
    items: [
      ...
      { name: "Контрастные процедуры", desc: "Чередование холода и тепла тренирует сосуды как спортзал. Микроциркуляция ускоряется, метаболизм включается на полную." },
      { name: "Термотерапия (сауна)", desc: "Пот уносит токсины, мышцы расслабляются, кровообращение восстанавливается. Сауна — древнейший и до сих пор лучший детокс." },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Гипокситерапия (ИГТ)", desc: "Тренировки в условиях мягкой гипоксии — организм учится эффективнее использовать кислород. Сердце, лёгкие и мозг работают лучше." },
    ],
  },
  {
    title: "Очистка и восстановление крови",
    color: "#b87a6a",
    items: [
      ...
      { name: "Плазмаферез", desc: "Фильтрация плазмы удаляет токсины, иммунные комплексы, лишний холестерин. Кровь становится чистой — тело начинает работать как новое." },
  ...
  },
  {
    ...
    color: "#d4622a",
    items: [
      { name: "HIIT и силовые тренировки", desc: "Короткие интенсивные подходы разгоняют метаболизм, силовые сохраняют мышцы молодыми. Без изнурения — с результатом." },
      { name: "Хот-йога и пилатес", desc: "Гибкость, баланс, глубокие мышцы — и при этом полный покой в голове. Тело учится быть лёгким." },
      { name: "Ходьба по лесу", desc: "Фитонциды деревьев, свежий воздух, мягкое движение — для мозга и нервной системы это лучшее лекарство." },
    ],
  },
  {
    ...
    color: "#9b7fb5",
    items: [
      { name: "Персонализированное питание", desc: "Кетодиета, интервальное голодание, FODMAP, палео — выбор основан на ваших анализах и целях, а не на модных трендах." },
      ...
      { name: "Митохондриальная поддержка", desc: "Коэнзим Q10, PQQ, альфа-липоевая кислота — топливо для ваших клеточных «электростанций». Энергия появляется без кофе и стимуляторов." },
    ],
  },
  {
    title: "Управление стрессом и ментальное здоровье",
    color: "#c8923a",
    items: [
      { name: "Медитация и майндфулнесс", desc: "Кортизол снижается, концентрация растёт — буквально за 8 недель практики мозг меняет свою структуру." },
      { name: "Метод Вима Хофа и пранаяма", desc: "Осознанное дыхание управляет вегетативной нервной системой. Стресс, холод, боль — становятся управляемыми." },
      { name: "Цифровой детокс и режим сна", desc: "7–9 часов в темноте, без голубого света вечером — и гормоны нормализуются, мозг восстанавливается, настроение выравнивается." },
    ],
  },
];
